...
namespace Engine
{
	FPSCameraComponent::FPSCameraComponent(float fov, float aspectRatio, float nearClip, float farClip, glm::vec3 position,
		...
		CameraComponent3D(fov, aspectRatio, nearClip, farClip, position, rotation, updateMousePress)
	{
		...
		m_componentType = ComponentType::FPSCamera; // Set the component type
	}

	void FPSCameraComponent::onAttach(GameObject* owner)
	{
		m_pOwner = owner;
	...
	void FPSCameraComponent::onUpdate(float timestep)
	{
	...
	void FPSCameraComponent::onEvent(Event& event)
	{
		// Create event dispatcher
		EventDispatcher dispatcher(event);
		// If the Event type matches, call the corresponding function
		dispatcher.dispatch<MouseButtonPressedEvent>(std::bind(&FPSCameraComponent::onMouseButtonPressed, this, std::placeholders::_1));
		dispatcher.dispatch<MouseButtonReleasedEvent>(std::bind(&FPSCameraComponent::onMouseButtonReleased, this, std::placeholders::_1));
	}

	bool FPSCameraComponent::onMouseButtonPressed(MouseButtonPressedEvent& e)
	{
	...
	bool FPSCameraComponent::onMouseButtonReleased(MouseButtonReleasedEvent& e)
	{
		// The event was the left mouse button being pressed
		if (e.getMouseButton() == ENGINE_MOUSE_BUTTON_LEFT)
		{
	...
	void FPSCameraComponent::updateView()
	{
	...
	void FPSCameraComponent::setPosAndRot(glm::vec3 position, glm::vec2 rotation)
	{
		m_position = position;
		m_fYaw = rotation.y;
		m_fPitch = rotation.x;
